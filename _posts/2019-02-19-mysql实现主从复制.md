...
- [基本流程](#基本流程)
- [docker-compose.yml文件配置](#docker-composeyml文件配置)
- [master配置](#master配置)
- [重启master](#重启master)
- [查看master状态](#查看master状态)
- [master库创建slave用户](#master库创建slave用户)
- [slave配置](#slave配置)
- [重启slave](#重启slave)
- [查看slave状态](#查看slave状态)
- [连接master服务器](#连接master服务器)
- [laravel中相关配置](#laravel中相关配置)
- [注意](#注意)
- [错误修复](#错误修复)
...
- 如果多个从库需要注意
  1. 数据目录`auto.cnf`中的`server-uuid`值不能相等
  2. 配置文件`my.cnf`中的`server_id`值不能相等
